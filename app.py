...
# --- TAB 4: REPORTS (Financial Accuracy Version) ---
with t4:
    ...
    if not s_df.empty:
        ...
        # Calculate Expenses for the SAME selected month
        ...
        # 2. Gross Earnings (SRP - Bought)
        # This is your product profit margin
        total_gross_earnings = ((m_sales_rep['srp'] - m_sales_rep['bought']) * m_sales_rep['qty']).sum()
        
        # 3. Net Profit (Earnings - Expenses)
        ...
        c4.metric("Net Profit", f"₱{net_profit:,.2f}", delta=f"{net_profit:,.2f}")

        if net_profit < 0:
            st.warning("⚠️ **Alert:** Your expenses are higher than your product earnings for this month!")

        st.write("---")
        st.subheader("📝 Daily Transaction Log")
        # ... (Rest of your daily log code remains the same) ...
# --- TAB 5: UTANG ---
with t5:
